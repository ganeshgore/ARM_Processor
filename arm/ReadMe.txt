...
The last part of the principle, the smaller the better, may impact THUMB code
...
and code base compatible with WinCE.
...
There are four directives to control the code generation state:
...
There are two directives to control the instruction set versions assembled:

	PROCESSOR imm	- imm is a bitmap value to selectively enable core CPU
			  instructions
	COPROCESSOR imm - imm is a bitmap value to selectively enable
			  coprocessor instructions

	PROCESSOR uses the following values:
	...
	COPROCESSOR uses the following values:
...
At start-up the default is to enable all instructions and all coprocessors. See
the files in the ARMDOC folder for details about which instructions are enabled
by each value. For example:
...
There are two extra predefined symbols: %c and %p. %c is the bitmap value of
the current coprocessor set enabled. %p is the bitmap value of the current
processor set enabled.
...
Address sizes are always 32 bits in ARM and THUMB states, you can't use any
address overrides.
...
v1.17 2010-Sep-08 - Added support to separate byte and rotation values for
		     immediates in shifter encoding
...
v1.08 2006-Jun-09 - Fixed erroneous error with implicit r0 based addressing
		  ...
		  - Generation of special encoding to allow user code to detect
		     unencodable immediates and then select an alternate coding
		     without causing assembly errors
