...
def press(event):
    ...
    if event.key=='a':
        ...
        key_press = True
    if event.key=='d':
        timestamp = timestamp + 1 if (timestamp<len(val_arr)/var_count)  else timestamp
        key_press = True
...
for val in var_arr:
    if val in var_lst:
        ...
        lbl1 = ax.text(xval[ind],yval[ind],val,fontsize=20,transform=ax.transAxes,color="blue",weight="bold")
        lbl_arr[count] =lbl1
        count = count + 1
        print ("Found " , val  )
    else:
        lbl1 = ax.text(0.01,0.1,"testValue",fontsize=20,transform=ax.transAxes,color="blue",weight="bold")
        print ("NOT Found " , val  )
        lbl_arr[count] = lbl1
        count = count + 1
